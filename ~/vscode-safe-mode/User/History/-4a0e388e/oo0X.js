...
const addFavorite = async (req, res) => {
  try {
    ...
    // Check if the recipe is already in favorites before adding it
    if (user.favorites.includes(recipeId)) {
      return res.status(400).json({ message: "Recipe is already in favorites." });
    }

    // Add recipe to favorites only if it's not already in the array
    user.favorites.push(recipeId); // Directly push as we already checked for duplicates
    await user.save();
