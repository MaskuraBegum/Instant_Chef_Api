...
const addFavorite = async (req, res) => {
  try {
    const { userId, uemail, recipe } = req.body;  // Assuming you are sending the full recipe object now

    // Find the recipe by its _id
    const recipeToAdd = await recipe.findById(recipe._id);
    if (!recipeToAdd) {
      return res.status(404).json({ message: "Recipe not found" });
    }
    ...
    if (!user) {
      user = new User({
        uid: userId,
        email: uemail,
        favorites: [recipeToAdd],  // Add the full recipe object to the favorites array
      });
      await user.save(); // Save the new user with the first favorite
    } else {
      // If the user exists, just add the new recipe object to the favorites
      if (!user.favorites.some(fav => fav._id.toString() === recipeToAdd._id.toString())) {
        user.favorites.push(recipeToAdd);  // Add the full recipe object to the favorites
    ...
    return res.status(200).json({
      ...
      favorites: user.favorites  // Return the full recipe objects in favorites
    });
  } catch (error) {
