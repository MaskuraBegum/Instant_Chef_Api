...
const addFavorite = async (req, res) => {
  try {
    const { userId, uemail, recipeId } = req.body;  // Expecting recipeId, not the full recipe object

    // Validate recipeId before proceeding
    if (!recipeId) {
    ...
    if (!user) {
      user = new User({
        uid: userId,
        email: uemail,
        favorites: [recipeData._id],  // Store only the recipe ID in the favorites array
      });
      await user.save(); // Save the new user with the first favorite
    } else {
      // If the user exists, check if the recipe is already in favorites
      if (!user.favorites.some(fav => fav.toString() === recipeData._id.toString())) {
        user.favorites.push(recipeData._id);  // Store the recipe ID in the favorites array
    ...
    return res.status(200).json({
      ...
      favorites: user.favorites  // Return the list of recipe IDs in favorites
    });
  } catch (error) {
