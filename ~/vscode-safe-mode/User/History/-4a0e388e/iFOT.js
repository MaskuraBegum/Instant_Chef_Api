...
const addFavorite = async (req, res) => {
  try {
    ...
    console.log("Received userId:", userId, "recipeId:", recipeId, "uemail:", uemail);

    // Find user by Firebase UID (use 'uid' field in the schema)
    const user = await User.findOne({ uid: userId });  // Adjust the field name if needed
    if (!user) {
    ...
    if (!recipe) {
      console.log("Recipe not found in the database.");
      return res.status(404).json({ message: "Recipe not found" });
    }
