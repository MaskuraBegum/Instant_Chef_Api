...
const addFavorite = async (req, res) => {
  try {
    const { userId, recipeId, uemail } = req.body;  // Use recipeId instead of full recipe object
    ...
    if (!user) {
      user = new User({
        uid: userId,
        email: uemail,
        favorites: [{ name: recipeId.name, description: recipeId.description, Ingredients: recipeId.Ingredients, instructions: recipeId.instructions, cookingTime: recipeId.cookingTime, category: recipeId.category, image: recipeId.image }]  // Add the first favorite in the required format
      });
      await user.save(); // Save the new user with the first favorite
    } else {
      // If the user exists, just add the new recipeId to the favorites
      if (!user.favorites.some(fav => fav.name === recipeId.name)) {
        // Add the new recipe in the required format
        user.favorites.push({
          name: recipeId.name,
