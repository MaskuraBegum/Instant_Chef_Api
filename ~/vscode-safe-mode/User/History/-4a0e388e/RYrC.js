...
const addFavorite = async (req, res) => {
  try {
    const { userId, recipe, uemail } = req.body;  // Use recipeId instead of full recipe object
    console.log(recipe);
    ...
    if (!user) {
      ...
      const isFavorite = user.favorites.some(fav => 
        ...
        fav.Ingredients.sort().toString() === recipe.Ingredients.sort().toString()  // Check if both name and ingredients are the same
      );
