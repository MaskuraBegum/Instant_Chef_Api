  ...
  try {
    ...
    if (!user) {
      user = new User({
        ...
        }]  // Add the first favorite in the required format
      });
      ...
      const isFavorite = user.favorites.some(fav => 
        fav.name === recipeId.name && 
        fav.Ingredients.sort().toString() === recipeId.Ingredients.sort().toString()  // Check if both name and ingredients are the same
      );
