...
const mongoose = require('mongoose'); // If using ObjectId, ensure mongoose is required
...
const addFavorite = async (req, res) => {
  try {
    ...
    // Convert recipeId to ObjectId if it's passed as a string (to match MongoDB format)
    const recipeObjectId = mongoose.Types.ObjectId(recipeId);

    // Use $addToSet to add the recipeId only if it doesn't already exist in the favorites array
    const updatedUser = await User.findOneAndUpdate(
      { uid: userId }, // Find the user by UID
      { $addToSet: { favorites: recipeObjectId } }, // Add recipeId only if it's not already in favorites
